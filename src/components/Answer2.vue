...
export default {
  ...
  methods: {
    ...
    // 问问题
    askQuestion () {
      // this.showQuestionIndex = Math.floor(Math.random() * 3)
      const qObj = this.data[this.question.answer[this.hasAnswered]['index']]
      let qText = `${qObj.qData[0].txt}?A.${qObj.as[0]}B.${qObj.as[1]}C.${qObj.as[2] ? qObj.as[2] : '都不是'}`
    ...
    checkAnswer () {
      ...
      this.hasAnswered ++
      if ((this.showQuestionIndex === this.rightAnswer && this.conversation[this.conversation.length - 1].text === 'true') ||
        (this.showQuestionIndex !== this.rightAnswer && this.conversation[this.conversation.length - 1].text === 'false')) {
        this.rightAnswers ++
